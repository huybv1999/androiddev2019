...
import vuhuy.kashima.protocol.IRCClient;
...
public class IRCConnection extends IRCClient
{
    ...
    {
        this.server = Huy.getInstance().getServerById(serverId);
    ...
    {
        @SuppressLint("StringFormatMatches") Message message = new Message(service.getString(R.string.message_set_channel_limit, sourceNick, limit));
        message.setColor(Message.COLOR_BLUE);
        server.getConversation(target).addMessage(message);
